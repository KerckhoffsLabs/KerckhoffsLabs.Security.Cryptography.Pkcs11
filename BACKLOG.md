...
- Total items: 156 (42 resolved, 114 open)
- Critical: 0 | High: 32 (16 open, 16 resolved) | Medium: 86 (70 open, 16 resolved) | Low: 38 (28 open, 10 resolved)
- Headline risks:
...
- **Problem:** 1005 of ~2060 test attributes are `ConditionalFact`/`ConditionalTheory`, plus 46 in-body `SkipTestException`. The three `PKCS11_TEST_EXPECT_*` guards assert *backend presence* only — never *mechanism* availability. `SupportsMlDsa`/`MlKem`/`SlhDsa` key on the advertised `C_GetMechanismList` (SoftHSM's `WITH_ML_DSA`/`WITH_ML_KEM` build flags gate the mechanism's entry into its own mechanism list, so this is accurate today), but nothing stops the *entire* mechanism list from coming back empty or truncated if the from-source OpenSSL 3.5 step degrades or the token init silently fails — every ML-DSA/ML-KEM test would skip and CI would still report green. All three test steps use `--logger "console;verbosity=normal"` only: no `.trx`, no skipped count, no minimum-executed floor. This is the systemic gap that allowed BL-053 to exist and would hide the next one.
