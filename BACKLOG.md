...
- **Resolution:** Added one real-module acceptance test per named priority, following the established `SignIbmMlDsaContextTests.OpenCryptoki.cs` pattern (a token that rejects a malformed parameter block is the oracle):
  ...
  - `IkeDeriveTestCases.cs` (`IkeDeriveTests.Nss.cs`) — all four IKE-derive mechanisms (`CkmIkePrfDeriveParams`, `CkmIke1PrfDeriveParams`, `CkmIke1ExtendedDeriveParams`, `CkmIke2PrfPlusDeriveParams`). NSS's `sftkike.c` is the only vendored implementation with real dispatch (confirmed genuine registration with `CKF_DERIVE` in `pkcs11.c:675-678`, unlike Kryoptic's debug-name-table-only match). Every one of NSS's four functions is a documented plain-HMAC construction (RFC 2409 §5/Appendix B, RFC 7296 §2.13), so each case reproduces that exact construction with the BCL's `HMACSHA256` as an independent reference and cross-checks byte-for-byte: `CKM_IKE_PRF_DERIVE` as `HMAC(inKey, Ni‖Nr)`, `CKM_IKE1_PRF_DERIVE` as `HMAC(inKey, gxy‖CKY_I‖CKY_R‖keyNumber)` (needs a second imported key for the `gxy` handle), `CKM_IKE1_EXTENDED_DERIVE` as the `K1=HMAC(K,extra)`, `K2=HMAC(K,K1‖extra)` chain (forced past the single-block subset shortcut via non-empty extra data, requesting two PRF blocks), and `CKM_IKE2_PRF_PLUS_DERIVE` as the RFC 7296 `T1=HMAC(K,S‖0x01)`, `T2=HMAC(K,T1‖S‖0x02)` construction truncated to a non-block-aligned length. Same `ExtractableDeriveAvailable` NSS gate as HKDF's cases (NSS's non-extractable-derive policy is general, not HKDF-specific).
