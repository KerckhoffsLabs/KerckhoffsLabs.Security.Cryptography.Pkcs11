...
### [BL-126] ⚠️ PARTIALLY RESOLVED — All four named priorities and HKDF landed; ChaCha20, IKE-derive, X3DH/X2Ratchet and XEdDSA still never reach a real module (and never can — no vendored backend implements them)
- **Area:** QA
- **Severity:** Medium
- **Effort:** L
...
- **Remaining scope:** `CkmChaCha20Params`, the IKE-derive family, X3DH/X2Ratchet and XEdDSA are still unit-marshalling-only, and a source audit of all five vendored backends (SoftHSM2, opencryptoki, Kryoptic, NSS, pkcs11-mock) found none of them implement the raw `CKM_CHACHA20` stream cipher, the IKE-derive family, or Signal's X3DH/X2Ratchet/XEdDSA — Kryoptic's apparent match on all of these is only a symbol-to-name debug table (`const_to_elem!` in `src/pkcs11/mod.rs`), not a real mechanism registration, and its own `chacha20.rs` says outright "`CKM_CHACHA20` is not implemented" (it only has the AEAD variant). No real-backend test is possible for any of these four until a vendored backend actually implements one — this is a hard floor on this item's remaining scope, not an effort gap.
- **Resolution:** Added one real-module acceptance test per named priority, following the established `SignIbmMlDsaContextTests.OpenCryptoki.cs` pattern (a token that rejects a malformed parameter block is the oracle):
  ...
  None of these run in this project's sandbox (opencryptoki/SoftHSM2/Kryoptic don't load there, and NSS's cases are permanently dormant per above), but all compile and are wired into the same skip/skip-unless machinery as every other real-backend test in the suite.
- **Breaks public API?** No
- **Raised by:** QA B
