...
### [BL-126] ⚠️ PARTIALLY RESOLVED — All four named priorities landed; the broader mechanism-parameter inventory (HKDF, ChaCha20, IKE-derive, X3DH/X2Ratchet, XEdDSA) still never reaches a real module
- **Area:** QA
- **Severity:** Medium
- **Effort:** L
...
- **Remaining scope:** All four named priorities now have real-backend coverage (see Resolution). What's left is the rest of the original inventory the Problem paragraph names: `CkmHkdfParams`, `CkmChaCha20Params`, the IKE-derive family, X3DH/X2Ratchet and XEdDSA are still unit-marshalling-only, plus every other untouched param type this item's Effort:L scope originally covered.
- **Resolution:** Added one real-module acceptance test per named priority, following the established `SignIbmMlDsaContextTests.OpenCryptoki.cs` pattern (a token that rejects a malformed parameter block is the oracle):
  ...
  None of these run in this project's sandbox (opencryptoki/SoftHSM2 don't load there), but all four compile and are wired into the same skip/skip-unless machinery as every other real-backend test in the suite.
- **Breaks public API?** No
- **Raised by:** QA B
