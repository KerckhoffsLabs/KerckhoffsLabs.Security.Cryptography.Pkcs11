...
- Total items: 160 (50 resolved, 110 open)
- Critical: 0 | High: 32 (13 open, 19 resolved) | Medium: 87 (67 open, 20 resolved) | Low: 41 (30 open, 11 resolved)
- Headline risks:
...
- **Severity:** Medium
- **Effort:** S
- **Location:** `src/KerckhoffsLabs.Security.Cryptography.Pkcs11/Native/LowLevelPkcs11Library.cs:1907-1944` (line numbers shifted by the BL-078 fix, which reordered `Dispose(bool)` but left the finalizer's `Dispose(false)` behavior — the thing this entry is about — unchanged: it still only sets `_disposed = true` and skips releasing `_library`); the claim at `Internal/SafeHandles/Pkcs11SessionHandle.cs:13-18`, release path `:43-61`; `Internal/SafeHandles/Pkcs11ModuleHandle.cs:29-41`
