...
- Total items: 156 (30 resolved, 126 open)
- Critical: 0 | High: 32 (28 open, 4 resolved) | Medium: 86 (70 open, 16 resolved) | Low: 38 (28 open, 10 resolved)
- Headline risks:
  ...
  - **Nine public-API decisions are cheap now and SemVer-major later.** Exceptions do not derive from `CryptographicException` so `catch (CryptographicException)` around the BCL-shaped façades silently fails; `ECCurve` collides with `System.Security.Cryptography.ECCurve` (the library needs four internal aliases to compile); 96 `CKF`/`CK`/`CKZ` constants publish the platform-width-dependent third-party `NativeCULong`; every `CK_VERSION` is a lossy string in which a v3.1 module renders as `"3.01"`; and `LoadStaticallyLinked()` cannot work on any shipped RID because `__Internal` is Mono-only (BL-063 – BL-071).
...
### [BL-063] Library exceptions do not derive from `CryptographicException`, so `catch (CryptographicException)` around the BCL-shaped façades does not work
- **Area:** .NET API Design
- **Severity:** High
- **Effort:** S
...
### [BL-064] `LoadStaticallyLinked()` cannot work on any RID the package ships — `__Internal` is a Mono-only convention
- **Area:** P/Invoke
- **Severity:** High
- **Effort:** S
...
### [BL-066] The public `ECCurve` collides with `System.Security.Cryptography.ECCurve` — the library's own code needs an alias to compile
- **Area:** .NET API Design
- **Severity:** High
- **Effort:** M
...
### [BL-067] `NativeCULong` — a third-party, platform-width-dependent type — is on the public surface via 96 `CKF`/`CK`/`CKZ` constants
- **Area:** .NET API Design
- **Severity:** High
- **Effort:** M
...
### [BL-068] Every `CK_VERSION` reaches the public API only as a lossy, ambiguous string — no programmatic version comparison is possible
- **Area:** .NET API Design
- **Severity:** High
- **Effort:** M
...
### [BL-076] `Pkcs11SessionHandle` narrows `CK_ULONG` through `IntPtr` in a checked context — a legal high-bit session handle throws and orphans the session
- **Area:** P/Invoke
- **Severity:** High
- **Effort:** S
...
### [BL-152] `ECCurve` lacks `[MemberNotNullWhen]`, forcing consumers to null-forgive `Oid`
- **Area:** .NET API Design
- **Severity:** Low
- **Effort:** S
- **Location:** `src/KerckhoffsLabs.Security.Cryptography.Pkcs11/ECCurve.cs:39` (`public string? Oid`), `:52` (`IsNamed`), `:55` (`IsDefault => Oid is null`)
