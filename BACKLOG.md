...
- Total items: 156 (31 resolved, 125 open)
- Critical: 0 | High: 32 (27 open, 5 resolved) | Medium: 86 (70 open, 16 resolved) | Low: 38 (28 open, 10 resolved)
- Headline risks:
  ...
  - **Nine public-API decisions are cheap now and SemVer-major later — one of them now settled.** Exceptions do not derive from `CryptographicException` so `catch (CryptographicException)` around the BCL-shaped façades silently fails; `ECCurve` collided with `System.Security.Cryptography.ECCurve` and is now `Pkcs11ECCurve`, with a reflection guard against the next such clash (BL-066, resolved); 96 `CKF`/`CK`/`CKZ` constants publish the platform-width-dependent third-party `NativeCULong`; every `CK_VERSION` is a lossy string in which a v3.1 module renders as `"3.01"`; and `LoadStaticallyLinked()` cannot work on any shipped RID because `__Internal` is Mono-only (BL-063 – BL-071).
