...
### [BL-126] ⚠️ PARTIALLY RESOLVED — All four named priorities, HKDF and ChaCha20 landed; IKE-derive, X3DH/X2Ratchet and XEdDSA still never reach a real module (and never can — no vendored backend implements them)
- **Area:** QA
- **Severity:** Medium
- **Effort:** L
...
- **Remaining scope:** The IKE-derive family, X3DH/X2Ratchet and XEdDSA are still unit-marshalling-only, and a source audit of all five vendored backends (SoftHSM2, opencryptoki, Kryoptic, NSS, pkcs11-mock) found none of them implement the IKE-derive family or Signal's X3DH/X2Ratchet/XEdDSA — no real mechanism registration exists for any of them anywhere. No real-backend test is possible for these three until a vendored backend actually implements one — this is a hard floor on this item's remaining scope, not an effort gap. (Raw `CKM_CHACHA20` was in this same "no backend implements it" state when this item was last written, but Kryoptic gained a real implementation on 2026-09-12 — see Resolution below; that finding is now stale for ChaCha20 specifically.)
