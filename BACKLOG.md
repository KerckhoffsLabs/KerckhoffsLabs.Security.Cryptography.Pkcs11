...
_Generated 2026-07-09, extended 2026-08-11 from a second full multi-specialist deep review (cryptography ×2, PKCS#11 v3.2 conformance ×2, .NET library design + P/Invoke ×2, QA & release engineering ×3). Both rounds deduplicated overlapping findings and re-verified every Critical/High citation against the source; the 2026-08-11 round added BL-063 – BL-156, corrected three coverage-matrix rows, and repaired this file's missing `## Low` heading. A 2026-09-15 staleness pass re-verified every open item's Problem claim against current source (no new findings raised): BL-028, BL-115 and BL-159 were closed out by independent feature work (Kryoptic backend coverage, a `SetPin` rewrite, and third-backend encapsulate/decapsulate coverage, respectively) that nobody had circled back to mark; BL-030, BL-034 and BL-126 were rescoped to their narrower remaining gap after partial fixes (a README quickstart, a `CONTRIBUTING.md` stub, and real-backend `CKM_EDDSA` coverage). This pass also found the item counts below had drifted from the file's actual contents (160 `### [BL-*]` headers exist, not 157 — the Low section alone has 41, not 38 — evidently from additions after BL-160 was appended without the tally being re-run); the counts are now a direct recount of the current file rather than carried forward. BL-108 (`DeriveSharedSecretEcdh`'s ungated `CKD_NULL`) was fixed and closed the same day: both overloads now refuse `CKD_NULL` without `AllowInsecure`._
...
- Headline risks:
  ...
  - **The secure-defaults story had three real holes; two are now closed.** The key generators granted `CKA_WRAP`+`CKA_DECRYPT` and `CKA_ENCRYPT`+`CKA_UNWRAP` on one key, the textbook wrap-oracle and unwrap-injection pair — fixed by splitting generation into role-specific helpers, each defaulting every capability attribute to `false` (BL-070, resolved). The ECDH peer public key was never curve-matched or point-validated before reaching the token, which was invalid-curve private-key recovery against a key the library sells as non-extractable — fixed by validating the peer's curve, coordinate lengths, and point-on-curve before every agreement (BL-080, resolved). Still open: the insecure-mechanism gate omits `CKM_EXTRACT_KEY_FROM_KEY` and the `CKM_CONCATENATE_*` family — Clulow's canonical key-extraction attack — in both the runtime gate and the shipped analyzer (BL-074).
  - **Session and library lifetime is not safe under concurrency, and only one workspace per slot actually works — one lifetime defect is now closed.** `LowLevelPkcs11Library.Dispose` unmapped the module *before* setting a non-`volatile` disposed flag; the flag is now `volatile` and set before the unmap (BL-078, resolved). `CloseAllSessions()` stranding handles a later finalizer could use to close an unrelated live session is likewise moot — the method was removed from the public surface (BL-071, resolved). Still open: `Pkcs11Library.Dispose()` closes tracked sessions outside the busy lock BL-015 added (BL-082); `C_Finalize` is decided per instance, so one instance tears down global state under another's live sessions (BL-083); and the second `OpenWorkspace` on a slot throws `CKR_USER_ALREADY_LOGGED_IN` while disposing either logs the other out (BL-084).
...
Three findings came closest and were deliberately held at High rather than inflated, because each requires a caller action or a non-default posture to become exploitable: **BL-075** (a caller-supplied bit-length that exceeds its own buffer makes the module read past an allocation — genuine out-of-bounds, but caller-triggered) — now resolved, both param types restrict the bit-length to the discrete widths their mechanism defines and check it against the buffer's actual length; **BL-080** (invalid-curve ECDH recovers a token-resident private key, but only through the raw-secret read-back path, which is itself `AllowInsecure`-gated) — now resolved, every agreement validates the peer's curve, coordinate lengths, and point-on-curve before it reaches the token; and **BL-070** (the wrap-oracle role conflict exfiltrates only a key the caller deliberately made extractable) — now resolved, key generation is split into role-specific helpers. All three of this round's near-Critical findings are now resolved.
...
- **Status:** Resolved 2026-08-11 by the entry's first option — fixed, not deprecated or deleted. The reviewer's diagnosis reproduced exactly: with the old bootstrap in place the new behavioural test fails with `DllNotFoundException: Unable to load shared library '__Internal'`, CoreCLR having gone looking for a real `__Internal.so`. The static branch of `Delegates(IntPtr)` is now `Load(ResolverFor(NativeLibrary.GetMainProgramHandle()))`, so the static path is the ordinary load sequence over a different handle rather than a separate bootstrap: same `C_GetFunctionList` entry, same best-effort v3.0/v3.2 binding, same graceful degradation for exports a v2.40-only module omits. That collapsed three things — the `NativeMethods` class, the parameterless `InitializeWithGetFunctionList()` overload, and the try/catch that previously swallowed a failure to reach the process symbol table (now load-bearing, because the bootstrap needs the same handle) — and dropped `partial` from `Delegates`, since the `[LibraryImport]` source generator was the only reason for it. Two tests, both verified to fail against the old code by restoring it: `LoadStaticallyLinked` on a host that exports no bootstrap reached the intended `EntryPointNotFoundException` naming `C_GetFunctionList` instead of failing earlier for the wrong reason (**since removed — see the follow-up below**), and a reflection guard asserts the assembly declares **zero** P/Invokes — the right invariant to hold, because everything else dispatches through the `CK_FUNCTION_LIST` pointer table, so any new P/Invoke is the shape this defect would return in whatever library name it named. The public XML doc no longer prescribes `DllImport("__Internal")`; it states the actual requirement (the host must export `C_GetFunctionList` from its entry-point module, via `DirectPInvoke` plus a linker export under Native AOT), documents `EntryPointNotFoundException`, and drops the unreachable iOS claim, since the package ships no `ios`/`maccatalyst`/`android` TFM. The stale internal comment on `LowLevelPkcs11Library`'s parameterless constructor was corrected too. Full suite green (2002 passed, 0 failed, 631 gated skips).
  ...
  - **Follow-up 2026-08-11 — the in-process negative test was removed, and finding out why was worth more than the test.** It asserted that a host exporting no bootstrap gets `EntryPointNotFoundException`. That held on Linux and failed intermittently on the macOS CI leg with "no exception was thrown": dyld resolves through the entry-point module permissively enough that a module some *other* test in the assembly had already `dlopen`ed satisfied the lookup. This was not a flaky assertion to be stabilised but an unsafe one — on the runs where it passed, the call genuinely bound and `C_Initialize`d somebody else's module, and `Assert.Throws<T>(Func<T>)` does not dispose its result, so it leaked an initialized library; had it disposed, `C_Finalize` would have run underneath the tests still using that module. A `[ConditionalFact]` was tried and rejected: its condition is evaluated at discovery, before any module is loaded, so the race survives it. The test is gone; the deterministic no-P/Invoke guard stays, and the AotSmoke `static` mode proves the positive case on both platforms, which is the stronger claim anyway. The platform difference itself is now documented on `LoadStaticallyLinked`: on macOS, in a process that has already loaded a module by path, this method can bind *that* module rather than failing, so a caller who means a specific module should use the path-taking constructor.
- **Area:** P/Invoke
- **Severity:** High
- **Effort:** S
...
### [BL-069] ✅ RESOLVED — `CkmSp800108KdfParams.AdditionalDerivedKeys` returns raw `ulong` handles that no public API can accept
- **Status:** Resolved 2026-09-14. `AdditionalDerivedKeys` now returns `IReadOnlyList<Pkcs11Key?>` (`null` for a slot the token left unwritten). The raw handles `AbsorbOutput` collects are unchanged internally, but a new internal `HydrateDerivedKeys(Pkcs11Workspace)` method turns them into real `Pkcs11Key` instances — called from `Pkcs11Key.Derive` right after a successful `C_DeriveKey`, the one place that has both the handles and the workspace needed to build them (`Pkcs11Session`, which performs the native call, is internal and has no workspace reference, so hydration could not live there). Each returned key is documented as caller-owned (dispose, and `Destroy()` if it should not outlive the call) — the same obligation `Derive`'s primary return already carries. Doc comments on the property spell out the ownership contract and the null-for-unwritten-slot behavior.
  The low-level marshalling unit tests (`Sp800108KdfTests`) exercise `BuildMarshalable`/`AbsorbOutput` directly with no live session or workspace, so they needed the raw handles independent of hydration; kept them working via a new internal-only `RawAdditionalDerivedKeyHandles` accessor, visible to the test assembly via the existing `InternalsVisibleTo`.
  Added `DeriveKeyAdditionalKeysTests` (Integration/Keys) for genuine end-to-end coverage — the managed in-process fake token (`ManagedSoftToken`) previously ignored `AdditionalDerivedKeys`/`AdditionalDerivedKeysPtr` entirely, so it was extended to actually derive and store sibling key objects (one continuous derived-key-material stream split across the primary key and every sibling in request order, per PKCS#11 v3.0 §2.42 — not independently re-derived) and write their handles into the `CK_DERIVED_KEY` slots, matching what a real token does. The new tests assert a sibling comes back as a real, readable `Pkcs11Key` and is destroyable, and that the property stays empty when no additional keys were requested. Full local Managed/Mock suite green (1764 passed, 0 failed among locally-runnable backends; SoftHSM/opencryptoki/Kryoptic/NSS backends are CI-only in this environment).
- **Area:** .NET API Design
- **Severity:** High
- **Effort:** M
...
### [BL-070] ✅ RESOLVED — The secure-defaults key generators grant conflicting wrap/decrypt roles on one key, enabling the classic wrap-oracle and unwrap-injection attacks
- **Status:** Resolved 2026-09-07. `GenerateAesKey` now grants `Encrypt().Decrypt()` only; a new `GenerateAesKeyEncryptionKey` helper produces a dedicated `Wrap().Unwrap()`-only KEK with `CKA_UNWRAP_TEMPLATE` imposing sensitive/non-extractable on everything it unwraps and `CKA_WRAP_TEMPLATE` filtering to sensitive keys. `GenerateRsaKeyPair` was removed and replaced with `GenerateRsaSigningKeyPair` (`Sign()`/`Verify()` only) and `GenerateRsaKeyTransportKeyPair` (`Encrypt()`/`Decrypt()` only, no wrap/unwrap on either) — two explicitly-named methods rather than reusing the old name for one role, so a caller relying on the old combined capability gets a compile error instead of a silent capability change.
  Testing against real NSS and SoftHSM tokens (not just the in-process managed fake) surfaced a second, more fundamental issue: PKCS#11's own spec default for an *omitted* capability attribute (`CKA_ENCRYPT`/`DECRYPT`/`SIGN`/`VERIFY`/`WRAP`/`UNWRAP`/`DERIVE`) is `CK_TRUE`, not `CK_FALSE` — simply leaving `.Wrap()`/`.Unwrap()` out of a template does not refuse those roles on a conformant token, it leaves them granted. Fixed at the root: `SecretKeyTemplateBuilder`, `PublicKeyTemplateBuilder`, and `PrivateKeyTemplateBuilder` now set every applicable capability attribute to `false` in their constructors, the same fail-safe-default pattern already used for `CKA_SENSITIVE`/`CKA_EXTRACTABLE`. This closes the gap for every `ObjectTemplate` builder consumer in the library, not just the four `Pkcs11Workspace` helpers, and let the now-redundant explicit `.Wrap(false)`/`.Encrypt(false)`-style negations at each call site (including a pre-existing one in `GenerateEcKeyPair`) be removed.
  Documented the role split and the CK_TRUE default in README's new "Key role separation" section. All call sites in the test suite that generated a key purely to sign/verify or purely to encrypt/decrypt were updated to the matching new helper; four `ObjectTemplateTests` assertions on the builders' preset attribute counts were updated for the new defaults. Full suite verified green (2264 passed, 0 failed, 604 skipped for unavailable backends) across Managed, Mock, NSS, and SoftHSM.
- **Area:** Cryptography
- **Severity:** High
- **Effort:** M
...
### [BL-072] ✅ RESOLVED — Asymmetric BCL façades never set `KeySizeValue`: `KeySize` is 0, `LegalKeySizes` throws, and ECDSA DER-format signing (CSR / certificate signing) is impossible
- **Status:** Resolved 2026-09-07. Each of the four asymmetric adapters now reads its key size in the constructor and assigns `KeySizeValue` plus a single-entry `LegalKeySizesValue` (min=max=actual size, since a token-resident key can't be resized, so a generic range the token may not support would misdescribe it): `RSAPkcs11` reads `CKA_MODULUS_BITS`, falling back to `CKA_MODULUS` byte length if the token doesn't expose it; `ECDsaPkcs11`/`ECDiffieHellmanPkcs11` read `CKA_EC_PARAMS`, resolve it to a curve via the existing `Pkcs11ECCurve.FromEcParams`, and look up field size from a new curated field-size-by-OID table (`Pkcs11ECCurve.FieldSizeBits`, internal) covering every curve in `NamedCurves`; `DSAPkcs11` reads `CKA_PRIME` byte length (no `CKA_PRIME_BITS` attribute exists in PKCS#11). All four are best-effort: an unreadable/unrecognized attribute leaves `KeySize` at the BCL base-class default (0) rather than throwing, matching the existing pattern in the symmetric adapters (`AesPkcs11.TryReadKeySizeBits` et al.). Fixing `KeySizeValue` transitively fixed the CSR scenario with no adapter changes beyond the constructor, since the BCL's `ECDsa` base class implements the DER-format `SignData`/`GetMaxSignatureSize` overloads in terms of `KeySize`. Added regression tests per adapter (`KeySize`/`LegalKeySizes` assertions on the Managed backend across RSA/DSA/ECDsa/ECDH, plus an `ECDsaPkcs11`-backed `CertificateRequest.CreateSigningRequest()` round-trip — the exact headline scenario this entry named).
- **Area:** .NET API Design
- **Severity:** High
- **Effort:** S
...
### [BL-073] ✅ RESOLVED — AES-CCM and ChaCha20-Poly1305 cannot fall back to the v2.40 path; their fallback code is unreachable on modules that advertise the message API but reject the mechanism
- **Status:** Resolved 2026-09-14. Added `catch (Pkcs11Exception ex) when (...)` around the message-API attempt at all four missing sites (`AesCcmPkcs11.Encrypt`/`Decrypt`, `ChaCha20Poly1305Pkcs11.Encrypt`/`Decrypt`), mirroring `AesGcmPkcs11`'s existing pattern exactly — the message-API attempt is unchanged, only the fallthrough-on-`CKR_FUNCTION_NOT_SUPPORTED` handling is new.
  The exception-matching predicate was lifted into a shared `Pkcs11MessageApiFallback.IsUnsupported(Pkcs11Exception)` helper (`Algorithms/Pkcs11MessageApiFallback.cs`) and all six sites (the two pre-existing GCM ones plus the four new ones) now reference it, so there is exactly one place that defines what "the module rejects this mechanism through the message API" means. A `try`/`catch`-wrapping delegate helper (the initial approach) does not work here: the message-API attempt takes and returns `Span<byte>`/`ReadOnlySpan<byte>` parameters, which cannot be captured by a lambda closure (CS9108) — so each site keeps its own explicit `try`/`catch` block, with only the filter condition shared.
  No new integration test was added: reproducing the exact failure mode (a module that reports `IsMessageApiSupported=true` at the session level but returns `CKR_FUNCTION_NOT_SUPPORTED` for this one mechanism's `C_Message[En|De]cryptInit`) would need either flipping the shared `ManagedSoftToken` fake's `IsMessageApiSupported` globally (contradicts the documented, intentional purpose of its existing `AesGcmPkcs11Tests.Managed`/`AesCcmPkcs11Tests.Managed`/`ChaCha20Poly1305Pkcs11Tests.Managed` suites, which exist specifically to exercise the v2.40-only path) or a new fake reimplementing the interface's full surface — disproportionate to this entry's Effort: S scope. All 72 Managed-backend AEAD tests still pass unchanged (the fallback path they already exercised is untouched); `dotnet format --verify-no-changes` and a full build are clean.
- **Area:** Cryptography
- **Severity:** High
- **Effort:** S
...
### [BL-074] ✅ RESOLVED — The insecure-mechanism gate omits the key-extraction derive family — the canonical PKCS#11 key-leakage attack is ungated
- **Status:** Resolved 2026-09-14. Added all three named groups to `GuardMechanism` and `InsecureMechanismData.GatedMechanisms`: **(a)** the key-extraction/derive family (`CKM_EXTRACT_KEY_FROM_KEY`, `CKM_XOR_BASE_AND_DATA`, `CKM_CONCATENATE_BASE_AND_KEY`/`_BASE_AND_DATA`/`_DATA_AND_BASE`) got its own message naming Clulow's attack specifically, distinct from the "weak algorithm" framing every other case uses, per the proposed action; **(b)** `CKM_MD5_HMAC`/`_HMAC_GENERAL`/`_KEY_DERIVATION`, `CKM_SHA1_KEY_DERIVATION`, `CKM_RSA_9796`, and `CKM_SSL3_MD5_MAC`/`CKM_SSL3_SHA1_MAC` closed the MD5/SHA-1 asymmetry; **(c)** `CKM_CAMELLIA_ECB`/`CKM_ARIA_ECB` joined the existing `CKM_AES_ECB` case (identical rationale), `CKM_IDEA_ECB`/`CKM_GOST28147_ECB` got a Sweet32-plus-ECB-leak message, `CKM_CHACHA20`/`CKM_SALSA20` (raw, unauthenticated) got their own message pointing at `CKM_CHACHA20_POLY1305`, and `CKM_AES_XTS` got its own message (disk-sector-only, not general-purpose) rather than folding into the unauthenticated-AES-modes group.
  The bidirectional `AnalyzerGateParityTests` (which drive `GuardMechanism` over every `CKM` value and diff against `InsecureMechanismData.GatedMechanisms`) confirm the two lists are in exact parity — no build-time changes needed since the compile-time analyzer's own tests already exist for this purpose. All 63 Managed/Mock-backend tests touching the gate, the analyzer, and this parity suite pass; `dotnet format --verify-no-changes` and a full build are clean. No existing test referenced any of the 16 newly-gated mechanism names, so nothing needed updating for the behavior change itself.
- **Area:** Cryptography
- **Severity:** High
- **Effort:** S
...
### [BL-080] ✅ RESOLVED — The ECDH peer public key is never validated or curve-matched before being handed to the token — invalid-curve private-key recovery
- **Status:** Resolved 2026-09-07. Took the proposed action, split across a new shared validator (`Pkcs11PublicKeyView.GetCurve`/`ValidatePeerEcKey`) rather than duplicating it between the two call sites: the peer's curve OID must equal the local key's `CKA_EC_PARAMS` curve; its coordinate lengths must match that curve's known field size (skipped only for a curve outside this library's field-size catalog, where the OID-equality check still pins the peer to that exact curve); and the point is round-tripped through `ECDiffieHellman.Create(peer)`, which both the OpenSSL and CNG backends reject for an off-curve point. All three checks throw `Pkcs11ArgumentException`. `ECDiffieHellmanPkcs11.DeriveRawSecret` now derives `fieldSize` from the local key's curve (falling back to the peer's now-validated coordinate length only for an uncatalogued curve), closing the second defect (truncation via a subclassed `ECDiffieHellmanPublicKey` reporting a short `Q.X`). Added a validating `ECParameters` overload of `Pkcs11Workspace.DeriveSharedSecretEcdh` — the raw-span overload cannot validate on its own (a DER octet string carries no curve identity) and is now documented as caller-validated, with a pointer to the new overload. `CkmEcdh1DeriveParams` itself was deliberately left unchanged: it's a low-level, spec-shaped mechanism-parameter wrapper, and curve validation belongs in the high-level façades that actually know what "the local key's curve" means. Added 4 unit tests against the in-process managed token (peer on a different curve, an off-curve point, a coordinate-length mismatch, and the two derivation entry points) and 2 integration tests against real SoftHSM (the new overload's happy path round-trips and agrees like the raw-span form; a cross-curve peer is rejected) — all existing ECDH tests, including same-curve BCL round-trips, pass unchanged.
- **Area:** Cryptography
- **Severity:** High
- **Effort:** M
...
- **Severity:** High
- **Effort:** M
- **Location:** `.github/workflows/ci.yml:338`, `:360`, `:447` (all three `dotnet test` invocations); gates at `src/KerckhoffsLabs.Security.Cryptography.Pkcs11.Tests/Support/Fixtures/IPkcs11Backend.cs` (`SupportsMlDsa`/`MlKem`/`SlhDsa` default members)
- **Problem:** 1005 of ~2060 test attributes are `[Fact]`/`[Theory]` with `SkipUnless`/`SkipWhen` (`ConditionalFact`/`ConditionalTheory` before the xUnit v3 migration — see BL-135), plus 46 in-body `Assert.Skip` calls (`SkipTestException` before that same migration). The three `PKCS11_TEST_EXPECT_*` guards assert *backend presence* only — never *mechanism* availability. `SupportsMlDsa`/`MlKem`/`SlhDsa` key on the advertised `C_GetMechanismList` (SoftHSM's `WITH_ML_DSA`/`WITH_ML_KEM` build flags gate the mechanism's entry into its own mechanism list, so this is accurate today), but nothing stops the *entire* mechanism list from coming back empty or truncated if the from-source OpenSSL 3.5 step degrades or the token init silently fails — every ML-DSA/ML-KEM test would skip and CI would still report green. All three test steps use plain console output only: no `.trx`, no skipped count, no minimum-executed floor. This is the systemic gap that allowed BL-053 to exist and would hide the next one.
- **Proposed action:** Emit a `.trx` report (e.g. `Microsoft.Testing.Extensions.TrxReport` + `dotnet test --report-trx`) and add a CI step that fails when the skipped count exceeds a checked-in per-leg baseline, or when total executed drops below a floor — MTP's own `--minimum-expected-tests`/`--zero-tests-policy strict` cover part of this natively. Add `PKCS11_TEST_EXPECT_MLDSA`/`_MLKEM`-style declarations mirroring the existing backend-availability guards so a lost PQC capability fails rather than skips.
- **Breaks public API?** No
- **Raised by:** QA B, QA A
...
### [BL-108] ✅ RESOLVED — `DeriveSharedSecretEcdh` accepted `CKD_NULL` with no gate, silently making the raw ECDH x-coordinate the AES key — and its own doc described the opposite behaviour
- **Area:** Cryptography
- **Severity:** Medium
- **Effort:** S
- **Location:** `src/KerckhoffsLabs.Security.Cryptography.Pkcs11/Pkcs11Workspace.cs:601-630` (raw-span overload, doc + gate) and `:648-675` (`ECParameters` overload, delegates to the same gate)
- **Problem:** The `kdf` parameter doc said "pass `CKD.CKD_NULL` to take the raw shared secret as the key material (do your own KDF off-token)", but the method does not return the secret — it derives an on-token AES key from it — so there was no off-token KDF step for the caller to perform. With `CKD_NULL` the resulting AES key *is* the raw x-coordinate, or a token-chosen truncation of it when the field is larger than the requested key length, which SP 800-56A forbids. `GuardMechanism` has no case for `CKM_ECDH1_DERIVE`, so nothing warned or refused.
- **Resolution:** Both overloads now throw `InsecureOperationException` (`CKM_ECDH1_DERIVE`) when `kdf == CKD.CKD_NULL` and `AllowInsecure` is `false` — checked before `CkmEcdh1DeriveParams` is even constructed, so it costs nothing on the (now-default) `CKD_SHA256_KDF` path. The doc comment on `kdf` was corrected on both overloads to state what `CKD_NULL` actually produces and that it never hands the caller a raw secret for an off-token KDF. Hermetic gate coverage added in `DeriveSharedSecretEcdhTests.Pkcs11Mock.cs` (gated-by-default, `AllowInsecureScope` bypass, default-KDF non-trigger); the four real-backend suites (`DeriveSharedSecretEcdhTests.{SoftHsm2,Kryoptic,Nss,OpenCryptoki}.cs`), which all deliberately exercise `CKD_NULL`, were updated to set `AllowInsecure = true` since that is now what they are testing against, not incidentally relying on.
- **Breaks public API?** No — the gate is a behavioural addition; existing callers passing `CKD_NULL` without `AllowInsecure` now get `InsecureOperationException` instead of a silently weak key, consistent with every other insecure-by-default mechanism in this library
- **Raised by:** Cryptographer B
...
### [BL-111] ✅ RESOLVED — Forgotten-dispose safety depended on `~LowLevelPkcs11Library` looking like dead code, and `Pkcs11SessionHandle`'s doc asserted a finalization guarantee that did not exist
- **Area:** P/Invoke
- **Severity:** Medium
- **Effort:** S
- **Location:** `src/KerckhoffsLabs.Security.Cryptography.Pkcs11/Native/LowLevelPkcs11Library.cs` (`ModuleHandle` accessor); `Internal/SafeHandles/Pkcs11SessionHandle.cs` (ref-count + corrected remark); `Internal/SafeHandles/Pkcs11ModuleHandle.cs` (unchanged — release path is the one now deferred)
- **Problem:** `Pkcs11SessionHandle` and `Pkcs11ModuleHandle` are both `SafeHandle` (hence `CriticalFinalizerObject`), and the CLR gives **no ordering guarantee among critical finalizers**. The session handle's class remark stated the opposite — that the strong `_library` reference "guarantees that `C_CloseSession` can still be called when this handle is finally released" — but reachability prevents *collection*, not *finalization*, so on the path where a consumer drops the library and an open session together, `Pkcs11ModuleHandle.ReleaseHandle` (`NativeLibrary.Free`) could run before the session handle calls `C_CloseSession` through the unmapped module. What actually prevented that was incidental: `~LowLevelPkcs11Library() => Dispose(false)` runs in the *non-critical* phase and sets `_disposed = true`, so the later `C_CloseSession` hit the `ObjectDisposedException` guard and was swallowed by `ReleaseHandle`'s `catch`. Because `Dispose(false)` released nothing, that finalizer read as dead code.
- **Resolution:** `LowLevelPkcs11Library` now exposes its `Pkcs11ModuleHandle` internally, and `Pkcs11SessionHandle`'s constructor takes a `DangerousAddRef` on it for the session's lifetime whenever the backing library is a real, natively-loaded `LowLevelPkcs11Library` (skipped for test doubles, which have no native module to protect, and for an already-invalid session id, which never reaches `C_CloseSession`); `ReleaseHandle` releases that ref in an outer `finally` that runs on every exit path, after `C_CloseSession` has had its chance to run. SafeHandle's own ref-counting now genuinely defers the module's release until the session is done with it, regardless of GC finalization order between the two objects — no longer incidental on `~LowLevelPkcs11Library`'s side effect. The misleading class remark was corrected to describe the actual mechanism. `~LowLevelPkcs11Library` itself is unchanged (not touching a SafeHandle-typed field from a non-critical finalizer is the correct pattern, not dead code — the field has its own critical finalizer). The secondary consequences named in the Problem (no `C_Finalize` before unmap; no stray-session close) are out of scope here and remain tracked under BL-083.
- **Breaks public API?** No
- **Raised by:** .NET Engineer B
- **Spec / References:** `CriticalFinalizerObject` — critical finalizers run after non-critical ones, with no relative order among themselves. Related to BL-078 (resolved — the `volatile`/ordering half; did not touch the finalizer path this entry is about) and BL-083

### [BL-112] ✅ RESOLVED — No protected-authentication-path login: `pPin = NULL_PTR` was unreachable even though `TokenFlags.ProtectedAuthenticationPath` is surfaced
- **Area:** PKCS#11 Conformance
- **Severity:** Medium
- **Effort:** S
- **Location:** `Internal/Pkcs11Session.cs` (`Login(CKU)`, `InitPin()`, `SetPin()`); `Pkcs11Slot.cs` (`InitToken(string)`); `Pkcs11Library.cs` (`OpenWorkspace(string, CKU)`); `Pkcs11Workspace.cs` (`SetPin()`, `InitPin()`)
- **Problem:** PKCS#11 signals "collect the PIN on the token's own pinpad" by passing `pPin = NULL_PTR, ulPinLen = 0`, and this repo's own interop docs say exactly that. But every public PIN-taking entry point required a non-null `SecurePin`, and `SecurePin` throws on an empty PIN, so the managed surface could never produce the NULL pointer. The plumbing was already correct — `Delegates.C_Login` maps a zero-length span to `NULL` via `fixed` — only the entry points blocked it. So the library told consumers their token has `CKF_PROTECTED_AUTHENTICATION_PATH` and then gave them no way to use it: a pinpad reader or an HSM configured for on-device PIN entry could not be logged into at all.
- **Resolution:** Added the no-PIN overloads at the session layer (`Pkcs11Session.Login(CKU)`, `InitPin()`, `SetPin()`), the slot layer (`Pkcs11Slot.InitToken(string label)`), and the actual public surface consumers reach (`Pkcs11Library.OpenWorkspace(string, CKU)`, `Pkcs11Workspace.SetPin()`/`InitPin()`) — each passes an empty span through to the native call rather than requiring a `SecurePin`, documented as requiring `TokenFlags.ProtectedAuthenticationPath`. `LoginUser` was left alone: it has no public entry point yet (that gap is BL-099), so a protected-auth-path overload of it would be unreachable regardless. Verified against pkcs11-mock: every new overload reaches the native call and gets back `CKR_ARGUMENTS_BAD` — the mock's own `NULL == pPin` check — proving the empty span reached the native call as a null pointer rather than being blocked by a managed guard.
- **Breaks public API?** No — additive, but was a day-one blocker for pinpad users
- **Raised by:** .NET Engineer B
...
### [BL-119] ✅ RESOLVED — `Pkcs11Session.CloseSession()` left `_disposed == false`, and the whole test suite used it instead of `Dispose`
- **Area:** QA
- **Severity:** Medium
- **Effort:** S
- **Location:** `src/KerckhoffsLabs.Security.Cryptography.Pkcs11/Internal/Pkcs11Session.cs` (method removed); 85 test call sites across 29 files migrated to `Dispose()`
- **Problem:** `CloseSession()` disposed the handle and nulled `_sessionHandle` but never set `_disposed`. The object therefore stayed "live": `_sessionId` degraded to `CK_INVALID_HANDLE` and every operation passed its `ObjectDisposedException` guard and issued native calls with session handle `0`, yielding `CKR_SESSION_HANDLE_INVALID` instead of the `ObjectDisposedException` the type's contract promises, while `SessionId` returned `0` rather than throwing. It had zero production callers and (by the time of this fix) 85 test call sites across 29 files, so the suite routinely tore sessions down through this half-closed path rather than through the production `Dispose` path that BL-015 hardened.
- **Resolution:** Took the stronger of the two proposed options: `CloseSession()` is deleted outright rather than patched, and every call site across the test suite now calls `Dispose()` instead — `Pkcs11Session.Dispose()` was already a strict superset of what `CloseSession()` did (same handle release, plus setting `_disposed`, plus doing it under the proper busy-lock via `Monitor.Enter` rather than the throw-on-contention `AcquireExclusive()` `CloseSession()` used). The one test that enumerated `CloseSession` in a table of "operations that must throw `ObjectDisposedException` after `Dispose()`" (`Pkcs11SessionTests.Operations_AfterDispose_ThrowObjectDisposed`) had that row removed since the method it names no longer exists; the other eight operations in that table are unaffected.
- **Breaks public API?** No — the type is internal
- **Raised by:** PKCS#11 Specialist B
...
### [BL-126] ⚠️ PARTIALLY RESOLVED — All four named priorities, HKDF, ChaCha20 and IKE-derive landed; X3DH/X2Ratchet and XEdDSA still never reach a real module (and never can — no vendored backend implements them)
- **Area:** QA
- **Severity:** Medium
- **Effort:** L
...
- **Problem:** Cross-referencing `MechanismParams/*.cs` against everything under `Integration/` and `Algorithms/`, only `CkmAesGcmParams`, `CkmAesCcmParams`, `CkmEcdh1DeriveParams`, `CkmRsaPkcsOaepParams`, `CkmRsaPkcsPssParams`, `CkmSalsa20ChaCha20Poly1305Params`, `CkmSp800108KdfParams` and the PQC sign params are ever used in a real call. `CkmEddsaParams` (Ed25519ph and context signing), `CkmHkdfParams`, `CkmChaCha20Params`, the IKE-derive family, X3DH/X2Ratchet and XEdDSA are unit-marshalling-only. Notably `Pkcs11MechanismMap.MlDsaHashSign` — a public v3.2 entry point that builds `CK_HASH_SIGN_ADDITIONAL_CONTEXT` — and every non-default `CkhHedge` value were tested only by asserting the returned `Mechanism`'s fields, never by a token that would reject a malformed block. RSA-OAEP with a label (`CKZ_DATA_SPECIFIED` + `pSourceData`) was likewise verified only structurally. (`SlhDsaHashSign`, named in the original proposed action alongside `MlDsaHashSign`, does not exist as a method in this codebase — there is no combined-hash SLH-DSA mapping to test.)
- **Remaining scope:** X3DH/X2Ratchet and XEdDSA are still unit-marshalling-only, and a source audit of all five vendored backends (SoftHSM2, opencryptoki, Kryoptic, NSS, pkcs11-mock) found none of them implement Signal's X3DH/X2Ratchet/XEdDSA — no real mechanism registration exists for any of them anywhere. No real-backend test is possible for these until a vendored backend actually implements one — this is a hard floor on this item's remaining scope, not an effort gap. (Raw `CKM_CHACHA20` and the IKE-derive family were in this same "no backend implements it" state when this item was last written; Kryoptic gained a real ChaCha20 implementation on 2026-09-12, and NSS's `sftkike.c` turned out to be a genuine, dispatched IKE-derive implementation — see Resolution below — so both findings are now stale for those two families specifically.)
- **Resolution:** Added one real-module acceptance test per named priority, following the established `SignIbmMlDsaContextTests.OpenCryptoki.cs` pattern (a token that rejects a malformed parameter block is the oracle):
  - `SignHashMlDsaTests.OpenCryptoki.cs` — `Pkcs11MechanismMap.MlDsaHashSign(SHA256)` round-trips against opencryptoki's `CKM_HASH_ML_DSA_SHA256`, feeding the raw message (confirmed against `mech_pqc.c` that the token hashes it internally for every combined mechanism except the bare `CKM_HASH_ML_DSA`).
  - `SignMlDsaDeterministicHedgeTests.OpenCryptoki.cs` — `CkhHedge.CKH_DETERMINISTIC_REQUIRED` against `CKM_ML_DSA`; opencryptoki maps this straight to OpenSSL's `OSSL_SIGNATURE_PARAM_DETERMINISTIC` (confirmed in `mech_openssl.c`), so the test asserts the actual observable effect — two signs of the same message produce byte-identical signatures — not just "didn't throw".
  - `EncryptRsaOaepLabelTests.SoftHsm2.cs` — `CkmRsaPkcsOaepParams` with a non-empty label, round-tripping and asserting a mismatched label fails to decrypt; SoftHSM2 genuinely feeds the label into `EVP_PKEY_CTX_set0_rsa_oaep_label` for both directions (confirmed in `OSSLRSA.cpp`), so this is real encoding behaviour, not a validated-and-discarded field.
  - `SignEdDsaContextTests.OpenCryptoki.cs` — `CkmEddsaParams` for both Ed25519ctx (non-empty context, asserting a signature verifies only under the same context) and Ed25519ph (prehash, asserting the same signature does not also verify as plain Ed25519); opencryptoki's `build_edwards_params` selects between all three OpenSSL "instances" based on exactly these fields (confirmed in `mech_openssl.c`).
  - `HkdfTestCases.cs` (`HkdfTests.Nss.cs` / `.Kryoptic.cs`) — `CkmHkdfParams` in all three modes (Extract-and-Expand, Expand-only, Extract-only), cross-checked byte-for-byte against the independent BCL `System.Security.Cryptography.HKDF` implementation rather than just round-tripping. NSS and Kryoptic are the only two vendored backends with a real HKDF implementation (SoftHSM2, opencryptoki and pkcs11-mock have none). The read-back cases are gated on the already-established, permanently-`false` `NssBackendFixture.ExtractableDeriveAvailable` (NSS refuses to derive a readable key at all — the same documented limitation `SP800108HmacCounterKdfPkcs11Tests.Nss.cs` already works around); Kryoptic runs normally. A second set of cases (`*_MatchesBclViaSignProbe`, same technique introduced for `IkeDeriveTestCases`) closes the NSS gap for real instead of leaving it dormant: derive the key non-extractable (`CKA_SIGN` instead of `CKA_EXTRACTABLE`), HMAC-sign a fixed probe message with it on the token, and compare against the MAC the independent BCL-derived bytes would produce for that message. Gated on plain `NssBackendFixture.NssAvailable`; verified locally against a real Kryoptic build (all 6 HKDF cases pass together).
  - `RawChaCha20TestCases.cs` (`RawChaCha20Tests.Kryoptic.cs` / `.Nss.cs`) — `CkmChaCha20Params` (`CKM_CHACHA20`), gated on the live mechanism list rather than a hard skip: Kryoptic gained a real implementation upstream on 2026-09-12 (`latchset/kryoptic#504`, now vendored here), and NSS's `sftk_Init`/`pkcs11c.c` already had one (`CKM_CHACHA20` alongside the legacy `CKM_NSS_CHACHA20_CTR`). RFC 8439 defines ChaCha20-Poly1305's ciphertext as the raw ChaCha20 keystream (starting at block counter 1) XORed with the plaintext, so the BCL's `ChaCha20Poly1305` doubles as an independent reference implementation for the raw cipher — a genuine known-answer cross-check, not just a round trip. A second test demonstrates the mechanism's lack of authentication directly: decrypting with the wrong block counter silently returns garbage instead of throwing, which is the documented reason `CKM_CHACHA20` is gated by default (see BL-074's resolution). SoftHSM2, opencryptoki and pkcs11-mock still implement neither, so those backends have no corresponding test file.
  - `IkeDeriveTestCases.cs` (`IkeDeriveTests.Nss.cs`, `IkeDeriveTests.Kryoptic.cs`) — all four IKE-derive mechanisms (`CkmIkePrfDeriveParams`, `CkmIke1PrfDeriveParams`, `CkmIke1ExtendedDeriveParams`, `CkmIke2PrfPlusDeriveParams`). NSS's `sftkike.c` has real dispatch (confirmed genuine registration with `CKF_DERIVE` in `pkcs11.c:675-678`); Kryoptic's vendored submodule pin has only the debug-name-table-only match (`const_to_elem!` in `src/pkcs11/mod.rs`), but a real implementation is now up for review upstream (`latchset/kryoptic#512`) — `IkeDeriveTests.Kryoptic.cs` is gated on the live mechanism list (`backend.Supports(CKM.CKM_IKE_PRF_DERIVE)`, same as `RawChaCha20TestCases`'s pattern before `#504` merged), so it skips cleanly today and starts running for real automatically once that PR merges and the submodule pin is bumped — verified locally by building the `#512` branch's `libkryoptic_pkcs11.so` directly and pointing `PKCS11_TEST_KRYOPTIC_LIBRARY` at it: all 4 cases pass. Every one of the four constructions is documented plain HMAC (RFC 2409 §5/Appendix B, RFC 7296 §2.13), so each case reproduces it with the BCL's `HMACSHA256` as an independent reference and cross-checks byte-for-byte: `CKM_IKE_PRF_DERIVE` as `HMAC(inKey, Ni‖Nr)`, `CKM_IKE1_PRF_DERIVE` as `HMAC(inKey, gxy‖CKY_I‖CKY_R‖keyNumber)` (needs a second imported key for the `gxy` handle), `CKM_IKE1_EXTENDED_DERIVE` as the `K1=HMAC(K,extra)`, `K2=HMAC(K,K1‖extra)` chain (forced past the single-block subset shortcut via non-empty extra data, requesting two PRF blocks), and `CKM_IKE2_PRF_PLUS_DERIVE` as the RFC 7296 `T1=HMAC(K,S‖0x01)`, `T2=HMAC(K,T1‖S‖0x02)` construction truncated to a non-block-aligned length. NSS's read-back cases are gated on `ExtractableDeriveAvailable` (NSS's non-extractable-derive policy is general, not HKDF-specific) — permanently dormant, which is why `IkeDeriveTestCases`'s missing `workspace.AllowInsecure = true` (needed to read the derived key back for the comparison, same as `HkdfTestCases`) went unnoticed until the Kryoptic run above actually executed the code and hit `InsecureOperationException`; fixed in all four cases. A second set of cases (`*_MatchesBclViaSignProbe`) closes the NSS gap for real instead of leaving it dormant: derive the key non-extractable (`CKA_SIGN` instead of `CKA_EXTRACTABLE`, default `CKA_SENSITIVE` — never triggers the `AllowInsecure` gate at all), HMAC-sign a fixed probe message with it on the token, and compare against the MAC the independent BCL reference bytes would produce for that same message — a mismatch in even one derived bit changes the MAC with overwhelming probability, so this is a genuine cross-check without ever reading `CKA_VALUE`. Mirrors how `DeriveSharedSecretEcdhTests` already proves ECDH-derived AES keys match via a cross-party AES-GCM round trip instead of reading the raw key. Gated on plain `NssBackendFixture.NssAvailable`, so these run for real wherever NSS is actually available (verified locally against the `#512` Kryoptic build too: all 4 pass there alongside the existing read-back cases).

  None of these run in this project's sandbox (opencryptoki/SoftHSM2/Kryoptic don't load there, and NSS is unavailable), but all compile and are wired into the same skip/skip-unless machinery as every other real-backend test in the suite.
- **Breaks public API?** No
- **Raised by:** QA B
...
### [BL-127] ✅ RESOLVED — SP800-108 correctness verification is gated off on the one backend that implements it, and the workaround does not require extraction
- **Status:** Resolved 2026-09-16. Added `SP800108HmacCounterKdfPkcs11TestCases.Assert_DeriveKey_MatchesBclViaSignProbe` — exactly the proposed action: derives a non-extractable `CKA_SIGN` generic-secret key via the façade's `DeriveKey(label, context, ObjectTemplate)` overload, HMAC-signs a fixed probe message with it on the token, and asserts the result against `HMACSHA256.HashData` over the independent `SP800108HmacCounterKdf.DeriveBytes` reference. Gated on plain `NssBackendFixture.NssAvailable` (not `ExtractableDeriveAvailable`), so it runs for real wherever NSS is actually available instead of staying dormant; also wired into `SP800108HmacCounterKdfPkcs11Tests.Kryoptic.cs` for a second real backend. Verified locally against a real Kryoptic build (9/9 SP800-108 cases pass together); zero regressions in the full local suite.

  The note "the same technique applies to any future KDF on a no-extract token" turned out to apply immediately, not just prospectively: the identical pattern (derive non-extractable + `CKA_SIGN`, sign a probe message, compare MACs against the independent-reference bytes) was applied the same day to `HkdfTestCases`'s NSS suite and `IkeDeriveTestCases`'s NSS suite (BL-126), both previously dormant on the exact same `ExtractableDeriveAvailable` gate for the exact same reason. All three closures share one helper shape (`DeriveAndProbe`/`ExpectedProbeMac` or equivalent), and all three were confirmed against real NSS in CI before landing here — the IKE-derive and HKDF probe tests already passed on the `ubuntu-latest` leg (verified via `gh run view` on the triggering CI runs) before this SP800-108 case was written, giving confidence the technique itself was already proven rather than untested when applied a third time.
- **Area:** QA
- **Severity:** Medium
- **Effort:** S
...
- **Problem:** The quickstart snippet is the flagship API demonstration (`Pkcs11Library` → `OpenWorkspace(slotLabel:, CKU, SecurePin)` → `GenerateRsaKeyPair(modulusBits:, label:)` → `RSAPkcs11.SignData`) and the wrap-hardening snippet chains eight template-builder methods. The reviewer verified all of them resolve against the current surface today, but nothing compiles them — so the first pre-1.0 rename silently ships a broken quickstart, and the API is explicitly still moving (BL-008, BL-038, BL-039 were all signature changes; BL-063 through BL-071 add more). **This already happened**: BL-070's fix removed `GenerateRsaKeyPair` and the quickstart snippet above kept calling it, uncaught until a manual review noticed — exactly the silent-break scenario this entry predicted.
- **Follow-up note 2026-09-07:** the README snippet was manually patched to `GenerateRsaSigningKeyPair` after the fact; this entry's proposed action (a compiled samples project) is what would have caught it automatically.
...
- **Breaks public API?** No
- **Raised by:** QA C
- **Status:** Resolved 2026-09-14. Migrated the test project from xUnit v2 + `Microsoft.DotNet.XUnitExtensions` to native xUnit v3 (`xunit.v3` 4.0.1): `[ConditionalFact]`/`[ConditionalTheory]` became `[Fact]`/`[Theory]` with `SkipUnless`/`SkipWhen` (+ the now-mandatory `Skip` message), and `throw new SkipTestException(...)` became `Assert.Skip(...)`. `Microsoft.DotNet.XUnitExtensions` and the `dotnet-eng` package source (`NuGet.config`) are both gone, so the release path no longer depends on any non-nuget.org feed. Also picked up along the way: `dotnet test` moved to the native Microsoft.Testing.Platform mode (`global.json`'s `test.runner`), `--filter "FullyQualifiedName~X"` became `-- --filter-query "/*/*/*X*/*"`, and `coverlet.collector` became `coverlet.MTP` (needs `UseMicrosoftTestingPlatformRunner=true` to register).
...
### [BL-160] ✅ RESOLVED — SoftHSM2 token-object paths exceeded Win32 MAX_PATH (260 chars), failing every on-disk object create on Windows
- **Area:** QA
- **Severity:** Medium
- **Effort:** L
- **Location:** `src/KerckhoffsLabs.Security.Cryptography.Pkcs11.Tests/Support/Fixtures/SoftHsmBackendFixture.cs` (token store path)
- **Status:** Resolved 2026-09-14. Root cause confirmed via SoftHSM2's own DEBUG-level log (temporarily enabled: `log.level = DEBUG` + `log.file`, captured in CI): every token-object path SoftHSM2 creates under Windows — `<build-output>\runtimes\win-x64\native\tokens\<token-uuid>\<object-uuid>.object` — is **278 characters**, 18 over the classic Win32 `MAX_PATH` (260), because GitHub Actions' checkout convention (`D:\a\<repo>\<repo>\...`) plus this project's own nested build output path (`src\...\bin\Release\net10.0\runtimes\win-x64\native\`) already consume 190 of those characters before SoftHSM2 appends its own `tokens\<36-char-uuid>\<36-char-uuid>.object` (88 more). The vendored SoftHSM2 Windows build opens object files via narrow, non-long-path-aware `fopen`/`CreateFileA`, so every `C_CreateObject`/`C_GenerateKey*` for a `CKA_TOKEN` object fails with `ERROR_PATH_NOT_FOUND` ("No such file or directory" → `CKR_GENERAL_ERROR`) — confirmed 100% deterministic with zero exceptions: every one of the 110 "Created new object" lines SoftHSM2 logged was immediately followed by that exact failure. Session objects (never written to disk) are unaffected, which is why some tests still passed and made the failure look selective rather than absolute. This was migration-adjacent, not migration-*caused*: the checkout path, build output structure, and `SupportsTokenObjects` default were all unchanged by the xUnit v3 migration (BL-135) and identical under the last green xUnit v2 Windows run — the same 278-character paths would have failed there too had the exact same object UUIDs been generated; whether v2 was actually exercising `CKA_TOKEN=true` creates on Windows at the same rate was not independently verified, but it no longer matters now that the real fix is in. Three other mechanisms were investigated and ruled out along the way, each with direct CI evidence rather than just reasoning: cross-collection concurrency (`--max-threads 1`, fully serial, reproduced the identical failures), Microsoft.Testing.Platform's full hosting entry point (reverting to the legacy console-runner reproduced the identical failures), and multiple `SoftHsmBackendFixture` instances (stderr instrumentation confirmed exactly one instance per run).
  **Fix:** on Windows only, `SoftHsmBackendFixture` now roots its token store at a short, OS-temp-based path (`Path.GetTempPath()` + an 8-character random suffix, e.g. `C:\Users\runneradmin\AppData\Local\Temp\sthsm-a1b2c3d4\`) instead of deep inside the build output tree, bringing every object path down to roughly 135 characters — comfortably under 260 regardless of how deep the repo checkout is. Linux/macOS are untouched (no `MAX_PATH` concept; the token store stays next to the library as before).
- **Breaks public API?** No
- **Raised by:** QA A
...
- **Proposed action:** None — this is a real capability gap in SoftHSM2 and opencryptoki, not a wrapper bug. `Rfc2898DeriveBytesPkcs11` (BCL-`Rfc2898DeriveBytes`-shaped, over `CkmPkcs5Pbkd2Params`) is the exposed API; its `PrfForHash` switch supports exactly SHA-1/256/384/512 — not SHA-224, even though Kryoptic and NSS both implement `CKP_PKCS5_PBKD2_HMAC_SHA224` — because `System.Security.Cryptography.Rfc2898DeriveBytes.Pbkdf2` has no SHA-224 support either (confirmed: `HashAlgorithmName` has no `SHA224` field, and passing a manually-constructed `HashAlgorithmName("SHA224")` throws `CryptographicException: 'SHA224' is not a known hash algorithm`). With no independent reference to cross-check against, excluding it is a deliberate scope match to the BCL type this wrapper mirrors, not a test gap. The test suite cross-checks Kryoptic and NSS output against `Rfc2898DeriveBytes.Pbkdf2` for all four PRFs it does support, and also exercises `GetBytes`'s stream-continuation semantics against the BCL. SoftHSM2/opencryptoki's test files exist for symmetry; only the cases that actually invoke `CKM_PKCS5_PBKD2` skip via `RequireMechanism` — the constructor/argument-validation cases never touch the token and genuinely pass on both (empirically confirmed: 12 of 24 pass, 12 skip on SoftHSM2). Revisit only if either project adds the mechanism upstream.
...
- **Corrected 2026-09-19:** the original wording overclaimed "every PRF both support" (SHA-224 is supported by Kryoptic/NSS but has no BCL reference to test against, so it's out of scope by design, not an oversight) and "skip everything via RequireMechanism" (only the cases that touch the token do); both re-verified against source and an actual test run.
...
- **Real-HSM handle recycling frequency** (PKCS#11 Specialist B): whether shipping HSMs recycle session and object handles aggressively enough for BL-116 to fire in practice. The spec permits it and nothing defends against it, which is why it was raised; the actual frequency is vendor-dependent and would need a real-device experiment. (BL-071's half of this question is moot now that `CloseAllSessions()` is resolved by removal — there is no bookkeeping gap left to fire.)
...
- ~~**`CertificateRequest.Create` with `ECDsaPkcs11`** (.NET Engineer A): the claim follows from a verified `SignData(…, Rfc3279DerSequence)` failure on a shim subclass plus .NET's ECDSA `X509SignatureGenerator` using that format, rather than from a live token.~~ Confirmed and closed by BL-072's fix: `ECDsaPkcs11Tests_Managed.CertificateRequest_Create_SigningRequest_Succeeds` is exactly this round-trip, run against the in-process managed token.
