...
- **Resolution:** Added one real-module acceptance test per named priority, following the established `SignIbmMlDsaContextTests.OpenCryptoki.cs` pattern (a token that rejects a malformed parameter block is the oracle):
  ...
  - `HkdfTestCases.cs` (`HkdfTests.Nss.cs` / `.Kryoptic.cs`) — `CkmHkdfParams` in all three modes (Extract-and-Expand, Expand-only, Extract-only), cross-checked byte-for-byte against the independent BCL `System.Security.Cryptography.HKDF` implementation rather than just round-tripping. NSS and Kryoptic are the only two vendored backends with a real HKDF implementation (SoftHSM2, opencryptoki and pkcs11-mock have none). The NSS cases are gated on the already-established, permanently-`false` `NssBackendFixture.ExtractableDeriveAvailable` (NSS refuses to derive a readable key at all — the same documented limitation `SP800108HmacCounterKdfPkcs11Tests.Nss.cs` already works around); Kryoptic runs normally.
