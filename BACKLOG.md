...
- **Status: Partially resolved (2026-05-20) — typed-cert surface back to design.** Added the generic find/read: a public `Pkcs11Object` view (object class, label, id, `GetValue()` reading `CKA_VALUE`, `Delete()`) plus `Pkcs11Workspace.FindObjects(ObjectTemplate filter)`. Unlike `FindKeys`/`HydrateKeyFromHandle` — which read `CKA_KEY_TYPE` and so genuinely throw on non-key objects — `FindObjects` reads only `CKA_CLASS`/`CKA_LABEL`/`CKA_ID`, so certificates and data objects are now enumerable and readable (SoftHSM test stores a cert, reads `GetValue()` == DER, deletes it). `FindKeys` is retained, not redundant (operation-capable `Pkcs11Key` + companion pairing). **Reverted, pending design:** an earlier pass added `Pkcs11Object.AsX509Certificate()`/`AsX509CertificateWithPrivateKey()` and `FindCertificates()`. Two problems: (1) bolting cert-typed conversions onto the *generic* `Pkcs11Object` is poor design (you'd then want key-typed ones too); the typed conversion belongs in a typed finder. (2) `AsX509CertificateWithPrivateKey` proved infeasible cross-platform — `X509Certificate2.CopyWithPrivateKey` on the OpenSSL backend (Linux/macOS) *exports* the private key, which a non-extractable HSM key can't satisfy (Windows/CNG can associate a handle, but the API is OS-specific). The certificate-as-BCL-type surface (e.g. `FindCertificates()` returning `X509Certificate2`, and how to sign with the cert's HSM key cross-platform) is being designed before re-implementation. Generic per-attribute read/write remains BL-063; clean public non-key *import* (`ImportKey` throws on a cert template) remains a separate follow-up.
- **Area:** .NET API Design
- **Severity:** Medium
- **Effort:** M
