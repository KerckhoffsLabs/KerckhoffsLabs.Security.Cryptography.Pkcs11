...
- Total items: 156 (41 resolved, 115 open)
- Critical: 0 | High: 32 (17 open, 15 resolved) | Medium: 86 (70 open, 16 resolved) | Low: 38 (28 open, 10 resolved)
- Headline risks:
  ...
  - **The secure-defaults story had three real holes; one is now closed.** The key generators granted `CKA_WRAP`+`CKA_DECRYPT` and `CKA_ENCRYPT`+`CKA_UNWRAP` on one key, the textbook wrap-oracle and unwrap-injection pair — fixed by splitting generation into role-specific helpers, each defaulting every capability attribute to `false` (BL-070, resolved). Still open: the insecure-mechanism gate omits `CKM_EXTRACT_KEY_FROM_KEY` and the `CKM_CONCATENATE_*` family — Clulow's canonical key-extraction attack — in both the runtime gate and the shipped analyzer (BL-074); and the ECDH peer public key is never curve-matched or point-validated before reaching the token, which is invalid-curve private-key recovery against a key the library sells as non-extractable (BL-080).
