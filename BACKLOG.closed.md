# Library Review Backlog — Closed Items

_Items resolved or marked won't-fix, moved out of [BACKLOG.md](BACKLOG.md). Grouped under their original severity headings, in original order._
## Critical
...
- **Status: Resolved (2026-06-05)** — policy decided and applied. Instead of a blanket project-specific `PKCS11NET001` marker on the whole v3.2 surface (the original proposal), the adapters now **mirror the BCL's experimental status**: in .NET 10 `MLDsa` and `MLKem` were *stabilized* (no longer `[Experimental]`), so `MLDsaPkcs11` / `MLKemPkcs11` are intentionally **not** marked; `SlhDsa` (FIPS 205) is still `[Experimental("SYSLIB5006")]`, so `SlhDsaPkcs11` now carries `[Experimental("SYSLIB5006", UrlFormat = "https://aka.ms/dotnet-warnings/{0}")]` — the same diagnostic ID/URL as the BCL, so a consumer opts into both with one suppression. Propagation is enforced by `TreatWarningsAsErrors` (every consumer of `SlhDsaPkcs11` had to opt in). Notes on the dropped sub-scope: the v3.2 KEM methods (`Pkcs11Key.EncapsulateKey`/`DecapsulateKey`) wrap no experimental BCL type, so under the mirror-the-BCL policy they stay unmarked; a separate `STABILITY.md` was not added (the policy *is* "mirror the BCL"). If a future BCL release re-stabilizes `SlhDsa`, drop the marker from `SlhDsaPkcs11` to match.
...
- **Status: Resolved (2026-06-04)** — both paths now run in CI. ECDH was covered earlier (see the prior update). RSA-OAEP is now exercised by the in-process managed token (`ManagedSoftToken`): `RSAPkcs11Tests_Managed.OaepEncryptDecrypt_RoundTrips_AndDecryptsBclCiphertext` (SHA-1 and SHA-256) drives `RSAPkcs11.Encrypt`/`Decrypt` over `CKM_RSA_PKCS_OAEP` — exercising the `CK_RSA_PKCS_OAEP_PARAMS` marshalling and the adapter's encrypt/decrypt path — and additionally decrypts a ciphertext the BCL produced with the token's exported public key. So the OAEP marshalling/encrypt path runs every build regardless of SoftHSM's `SoftHsmSupportsOaepSha256 = false` (which still gates the *SoftHSM* KAT and is left as documentation of that token's limitation).
...
- **Status: Resolved (2026-06-05)** — `macos-latest` (Apple Silicon, arm64) was added to the CI matrix and runs the full suite (pkcs11-mock + SoftHSM2), so the arm64 ABI / `CK_ULONG` width and struct packing are now exercised on every push, closing the "regressions invisible" gap. The Windows leg also genuinely runs now (SoftHSM2 builds and the tests execute — see BL-049). Dedicated ARM64-Linux runners were not added, but the arm64 ABI is covered by Apple Silicon macOS.
- **Area:** QA / Release Eng
- **Severity:** High
- **Effort:** M
...
- **Validated (2026-06-05):** Windows-x64 now **builds and uses** SoftHSM2 in CI — the `_SoftHsm` real-crypto suites (AES-CCM/GCM, ChaCha20-Poly1305, DES/3DES, DSA, ECDSA, ECDH, HMAC, …) run and pass on the `windows-latest` leg, and the `SoftHsm_IsAvailable_OnCiBuildPlatforms` guard now **enforces** on Linux + Windows-x64 (the Windows exemption was dropped; macOS and the Windows-x86 leg stay exempt — the x86 testhost can't load the x64 library). Iteration took four fixes: (1) gate the SoftHSM build to Linux + Windows-x64 (macOS/x86 run pkcs11-mock only); (2) copy the vcpkg OpenSSL runtime DLLs (`libcrypto-*.dll`/`libssl-*.dll`) next to `libsofthsm2.dll` in `build-softhsmv2.ps1` (robust search + fail-loud); (3) split the vcpkg-OpenSSL `actions/cache` into restore + `if: always()` save so the ~27 min cold build persists across the (initially failing) iteration runs; (4) **the real `LoadLibraryA 0x7E` cause** — `SoftHsmBackendFixture.RunUtil` invoked `softhsm2-util` without `--module`, so it loaded a module path compiled in at build time (a CMake install prefix absent in CI); passing `--module "<built libsofthsm2>"` fixed it (Linux only worked because its autotools install dir persists in the workspace). Commits `2795aaf`, `d25d744`, `0e0522b`, `30d319a`, `fdc6de7`. The `macos-latest` leg was added in the same arc and made green by using the native arm64 SDK (a forced-x64 SDK ran under Rosetta and mis-placed pkcs11-mock under `osx-x64`) and gating platform-unsupported managed-token cases (AES-CCM, DSA-2048, NIST P-224, GCM sub-16-byte tags) on `*.IsSupported`/probe checks.
- **Extended (2026-06-05):** the x64 snapshot above was then extended to cover every leg. (a) **macOS** now also builds and uses SoftHSM2 (Homebrew autotools/OpenSSL 3; libsofthsm2 is a libtool `-module` `.so` bundle on macOS, not `.dylib`), with the availability guard enforcing on macOS too — commits `71887af`, `204198c`, `e96993b`. (b) **Windows ML-DSA is now wired up** (supersedes the "CMake path is ML-DSA-incapable" note in `build-softhsmv2.ps1`): SoftHSM 2.7's CMake never set `WITH_ML_DSA` (CompilerOptions.cmake has ECC/EDDSA OpenSSL-probe blocks but none for ML-DSA) and its crypto target omitted the `slot_mgr`/`object_store` includes the ML-DSA sources need. Forcing `-DWITH_ML_DSA=1` (the `config.h.in.cmake` slot and the sources already existed; vcpkg OpenSSL is 3.6+) plus injecting those include dirs via `/I` makes the Windows token ML-DSA-capable; the on-token `MLDsaPkcs11Tests_SoftHsm` cases (ML-DSA-44/65/87 sign+verify, no BCL cross-check) run and pass on `windows-latest` — commits `1f3e6e7`, `1797b44`. (c) The **Windows-x86** leg was un-gated by parametrizing `build-softhsmv2.ps1` by target RID (`win-x86` → `x86-windows` triplet / `Win32` generator), passed from the MSBuild target's `_SoftHsmRid`; the guard now enforces on all four legs — commit `e8d900e`.
- **Area:** Release Eng
- **Severity:** Medium
- **Effort:** S
...
- **Status: Resolved (2026-06-05)** — `IvBits` is now marshalled as `0` (PKCS#11 v3.2 §2.5.13 retains it as a legacy field whose value may be 0; the IV length is taken from `IvLen`). The stale `// TODO` was replaced with a doc comment citing §2.5.13 and noting that some tokens reject a non-zero value. The marshal round-trip test now asserts `IvBits == 0`; AES-GCM continues to pass against both pkcs11-mock and SoftHSM2.
- **Area:** Cryptography
- **Severity:** Medium
- **Effort:** S
