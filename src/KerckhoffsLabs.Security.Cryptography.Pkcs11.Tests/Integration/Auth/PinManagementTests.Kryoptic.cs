...
/// <summary>
/// Kryoptic integration for <c>Pkcs11Workspace.SetPinWithPin</c> — performs a real <c>C_SetPIN</c>
/// round-trip and always restores the shared token's user PIN.
/// </summary>
[Collection("Kryoptic")]
public sealed class PinManagementTests_Kryoptic(KryopticBackendFixture f)
{
    ...
    public void SetPinWithPin_ChangesUserPin_RoundTrip()
    {
        ...
        finally
        {
            if (atTemp)
            {
                using var o = new SecurePin(temp);
                using var n = new SecurePin(original);
                try { workspace.SetPinWithPin(o, n); } catch { /* best-effort restore of shared token */ }
            }
        }

        // Reaching here with atTemp == false proves both changes succeeded: the restoring SetPinWithPin
