...
/// <summary>
/// opencryptoki counterpart of FindObjectsTests_SoftHsm: enumerate/read/delete a non-key object (an
...
public sealed class FindObjectsTests_OpenCryptoki(OpenCryptokiBackendFixture backend)
{
    ...
    public void FindObjects_ReadsAndDeletes_CertificateObject()
    {
        ...
        using (var filter = ObjectTemplate.Empty().Label(label).Build())
        {
            ...
            finally { foreach (var o in objs) o.Dispose(); }
        }
    ...
    public void FindCertificates_BridgesToTokenPrivateKey_AndSigns()
    {
        ...
        finally
        {
            using (var cleanup = ObjectTemplate.Empty().Label(certLabel).Build())
                foreach (var o in workspace.FindObjects(cleanup)) { o.Delete(); o.Dispose(); }
            keypair.Delete();
        }
    }
}
