...
internal abstract class NotSupportedPkcs11Library : ILowLevelPkcs11Library
{
    ...
    public virtual CKR C_EncryptMessage(NativeCULong session, IntPtr parameter, NativeCULong parameterLen, ReadOnlySpan<byte> associatedData, ReadOnlySpan<byte> plaintext, byte[]? ciphertext, out NativeCULong ciphertextLen) { ciphertextLen = (NativeCULong)0; return CKR.CKR_FUNCTION_NOT_SUPPORTED; }
    ...
    public virtual CKR C_DecryptMessage(NativeCULong session, IntPtr parameter, NativeCULong parameterLen, ReadOnlySpan<byte> associatedData, ReadOnlySpan<byte> ciphertext, byte[]? plaintext, out NativeCULong plaintextLen) { plaintextLen = (NativeCULong)0; return CKR.CKR_FUNCTION_NOT_SUPPORTED; }
