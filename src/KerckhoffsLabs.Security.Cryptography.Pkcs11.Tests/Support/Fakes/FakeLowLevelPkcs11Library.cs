...
internal class FakeLowLevelPkcs11Library : ILowLevelPkcs11Library
{
    ...
    public virtual CKR C_EncryptMessage(NativeCULong session, IntPtr parameter, NativeCULong parameterLen, ReadOnlySpan<byte> associatedData, ReadOnlySpan<byte> plaintext, byte[]? ciphertext, out NativeCULong ciphertextLen) => throw new NotSupportedException("C_EncryptMessage");
    ...
    public virtual CKR C_DecryptMessage(NativeCULong session, IntPtr parameter, NativeCULong parameterLen, ReadOnlySpan<byte> associatedData, ReadOnlySpan<byte> ciphertext, byte[]? plaintext, out NativeCULong plaintextLen) => throw new NotSupportedException("C_DecryptMessage");
