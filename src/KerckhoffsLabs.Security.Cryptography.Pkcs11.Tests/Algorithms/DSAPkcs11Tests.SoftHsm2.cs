...
/// <summary>
/// DSAPkcs11 over SoftHSM. SoftHSM implements DSA (<c>CKM_DSA</c> / <c>CKM_DSA_SHA*</c>), so the
/// sign/verify and parameter-export tests run here. A keypair is generated on the token from a fixed
/// FIPS 186-3 2048/256 domain (p, q, g); signatures are cross-checked against the BCL <see cref="DSA"/>
/// using the public key exported from the token.
/// </summary>
[Collection("SoftHsm")]
public sealed class DSAPkcs11Tests_SoftHsm(SoftHsmBackendFixture f)
{
    ...
    [ConditionalFact(nameof(SoftHsmAvailable))]
    public void ImportParameters_NotSupported() => WithDsa(dsa =>
