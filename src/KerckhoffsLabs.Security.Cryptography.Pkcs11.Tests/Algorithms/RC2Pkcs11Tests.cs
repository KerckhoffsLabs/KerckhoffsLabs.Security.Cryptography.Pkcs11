...
/// <summary>
/// RC2Pkcs11 over SoftHSM. The non-RC2-key constructor check runs (it only needs an AES key). RC2
/// itself is unsupported by SoftHSM (no CKM_RC2_*), so every test that needs a token-resident RC2 key
/// skips here and runs against an RC2-capable token. Known-answer round-trips compare against the BCL
/// <see cref="RC2"/> for the same key, IV and effective key size.
/// </summary>
[Collection("SoftHsm")]
public sealed class RC2Pkcs11Tests_SoftHsm(SoftHsmBackendFixture f)
{
    ...
    // SoftHSM implements no RC2; skip RC2-key tests when the mechanism is absent.
    private void RequireRc2()
    {
        if (!_backend.Supports(CKM.CKM_RC2_CBC))
            throw new SkipTestException("Token does not implement CKM_RC2_CBC (SoftHSM has no RC2 support).");
    }

    // Imports Key128 as a token RC2 key and hands a wrapping RC2Pkcs11 (and its workspace) to the body.
    private void WithImportedRc2(Action<Pkcs11Workspace, RC2Pkcs11> body)
    {
        RequireRc2();
    ...
    // === Secure-defaults gate (requires RC2 support to construct the wrapper) ======================

    [ConditionalFact(nameof(SoftHsmAvailable))]
    public void EncryptCbc_Pkcs7_GatedByDefault_Throws() => WithImportedRc2((_, rc2) =>
        Assert.Throws<InsecureOperationException>(() => rc2.EncryptCbc(new byte[8], Iv8)));

    [ConditionalFact(nameof(SoftHsmAvailable))]
    public void EncryptEcb_GatedByDefault_Throws() => WithImportedRc2((_, rc2) =>
        Assert.Throws<InsecureOperationException>(() => rc2.EncryptEcb(new byte[8], PaddingMode.None)));

    // === Known-answer round-trips vs the BCL =======================================================
    ...
    // === NotSupported / lifecycle surface ==========================================================

    [ConditionalFact(nameof(SoftHsmAvailable))]
    public void EncryptCbc_UnsupportedPadding_Throws() => WithImportedRc2((_, rc2) =>
        Assert.Throws<NotSupportedException>(() => rc2.EncryptCbc(new byte[8], Iv8, PaddingMode.Zeros)));

    [ConditionalFact(nameof(SoftHsmAvailable))]
    public void GenerateIV_ProducesBlockSizedIv() => WithImportedRc2((_, rc2) =>
    {
        rc2.GenerateIV();
