...
/// <summary>SHA-384 digest against the second real backend (opencryptoki), cross-checked against the BCL.</summary>
...
public sealed class SHA384Pkcs11Tests_OpenCryptoki(OpenCryptokiBackendFixture backend)
{
    ...
    {
        if (!_backend.Supports(CKM.CKM_SHA384))
            throw new SkipTestException("opencryptoki: CKM_SHA384 not available");
