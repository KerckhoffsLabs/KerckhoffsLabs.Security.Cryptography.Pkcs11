...
/// <summary>
/// SHA3_256Pkcs11 over SoftHSM: token-computed SHA3-256 must match the FIPS 202 vector and the BCL.
/// Not every token implements <c>CKM_SHA3_256</c> (SoftHSM does not), so the known-answer hashes skip
/// when the mechanism is absent and run against a SHA3-capable token.
/// </summary>
[Collection("SoftHsm")]
public sealed class SHA3_256Pkcs11Tests_SoftHsm(SoftHsmBackendFixture f)
{
    ...
    private void RequireSha3()
    {
        if (!_backend.Supports(CKM.CKM_SHA3_256))
            throw new SkipTestException("Token does not implement CKM_SHA3_256 (SoftHSM has no SHA3 support).");
    }
