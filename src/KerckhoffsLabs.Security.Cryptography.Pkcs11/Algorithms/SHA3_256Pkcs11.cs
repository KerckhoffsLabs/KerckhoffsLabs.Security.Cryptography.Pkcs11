...
/// <summary>
/// BCL-aligned <see cref="SHA3_256"/> implementation backed by a PKCS#11 token's digest mechanism
/// (<c>CKM_SHA3_256</c>). Hashing happens on the token rather than in managed code. Does NOT take
...
/// on finalization (mirrors <see cref="SHA256Pkcs11"/>).
/// </para>
/// <para>
/// SHA3-256 (FIPS 202) is a secure hash and is not gated by the secure-defaults policy. The token must
/// implement <c>CKM_SHA3_256</c> — not all do (SoftHSM, for example, does not). Memory usage grows
/// linearly with accumulated input; the workspace is borrowed and not disposed.
/// </para>
/// </remarks>
public sealed class SHA3_256Pkcs11 : SHA3_256
{
    private readonly Pkcs11Workspace _workspace;
    private readonly System.IO.MemoryStream _buffer = new();

    /// <summary>
    /// Initialises a new PKCS#11-backed SHA3-256 instance over <paramref name="workspace"/>.
    /// </summary>
    /// <param name="workspace">An open workspace whose token supports <c>CKM_SHA3_256</c>. Borrowed,
    ...
    public SHA3_256Pkcs11(Pkcs11Workspace workspace)
    {
        ArgumentNullException.ThrowIfNull(workspace);
        _workspace = workspace;
        // SHA3_256's base constructor already fixes HashSizeValue at 256.
    }
    ...
    {
        using var mech = new Mechanism(CKM.CKM_SHA3_256);
