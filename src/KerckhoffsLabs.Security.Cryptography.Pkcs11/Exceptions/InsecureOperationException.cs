...
/// unless the caller has opted in via <c>Pkcs11Workspace.AllowInsecure = true</c> (or
/// <c>Pkcs11Workspace.AllowInsecureScope()</c>). Covers RSA PKCS#1 v1.5 padding (for both
...
public sealed class InsecureOperationException : Exception
{
    ...
    public InsecureOperationException(CKM mechanism, string suggestion)
        : base($"Mechanism {mechanism} is disallowed by default. {suggestion} " +
               $"To bypass, set Pkcs11Workspace.AllowInsecure = true (or scope it with " +
               $"Pkcs11Workspace.AllowInsecureScope()) before invoking the operation.")
    {
        Mechanism = mechanism;
    }
