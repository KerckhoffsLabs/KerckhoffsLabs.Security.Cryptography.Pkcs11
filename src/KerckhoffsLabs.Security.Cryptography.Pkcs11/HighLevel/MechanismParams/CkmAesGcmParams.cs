...
/// for the IV and AAD. Caller must dispose AFTER the <see cref="Mechanism"/> that
/// references it.
/// </summary>
public sealed class CkmAesGcmParams : IMechanismParams
{
    ...
    /// <param name="tagBits">Authentication tag length in bits; typically 128.</param>
    public CkmAesGcmParams(ReadOnlySpan<byte> iv, ReadOnlySpan<byte> aad, int tagBits)
    {
        ...
        if (tagBits <= 0 || tagBits > 128) throw new ArgumentOutOfRangeException(nameof(tagBits), "Tag size must be 1..128 bits.");
